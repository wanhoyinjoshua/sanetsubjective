   ...
   if(agitation==0){
    ...
    else if (agitation<5&&end!=true)
    {
      const interval = setInterval(() => {
        setagitation((prevCount) => prevCount + 0.0001);
      }, 1000); // Update the count every 1000ms (1 second)
...
function startaudio(){
  ...
  return (
    <div className="flex">
      ...
      {agitation<100?  <button className="bg-red-500 hover:bg-red-600 text-white py-2 px-4 rounded" onClick={endsimulation}>End Conversation</button>:null}
      ...
      </section>:<section className="w-2/3 p-4">
      ...
      <form onSubmit={(e)=>{submithandler(e)}}>
        ...
        <label>
          ...
          <input className='fixed bottom-10000' type='number' value={agitation} name="agitation" ></input>
          
        </label>
        <section className="fixed grid grid-cols-3 gap-4   bottom-0 border border-gray-300 rounded mb-8 shadow-xl p-2">
        ...
        <input
          ...
          />
        <button type="submit" class="bg-gray-300 ">Send</button>
          </section>
